...
module.exports = function(rate){
  ...
  o.play = function(from,to,rate){
    ...
    function fn(){
      ...
      setTimeout(function(){
        ...
        looping = false;
        console.log('current frame',frame,'end frame',end) 
  ...
  o.writeStream = function(source){
    ...
    return through(function(change){
      ...
      /*if(frame.index && frame.t) {
        if(z.frames[frame.index]) {
          if(z.frames[frame.index].id == frame.id){
            z.frames[frame.index] = frame;// frame change at same position
            z.frameids[frame.id] = frame;
          } else if(z.frames[frame.index].t >){
      ...
      */
      console.log('frameset writeStream ',frame)
      z.frames.push(frame);
      z.frameids[frame.id] = frame;
